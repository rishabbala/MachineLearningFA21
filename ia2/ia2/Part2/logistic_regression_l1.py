...
# reg_lambda_range = [0.0001, 0.00031622776601683794, 0.01]
training_limit = 1000
...
for reg_lambda in reg_lambda_range:
    ...
    sparsity.append(np.sum(np.where(abs(w) <= 10**-4, 1, 0)))
    with open ("Part2.csv", 'a') as file:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.title(f"L1 Lambda = {reg_lambda}")
    plt.legend(["Train Accuracy", "Dev Accuracy"])
    plt.pause(1)
    plt.savefig(f"L1 lamda = {reg_lambda}.png")
...
ax.set_xscale('log')
plt.title(f"L1 Accuracy vs Lambda")
plt.xlabel("Lambda")
plt.ylabel("Accuracy")
plt.legend(["Train Accuracy", "Dev Accuracy"])
plt.pause(1)
plt.savefig(f"L1 Accuracy vs lambda.png")
...
plt.title(f"Sparsity of Weights for L1 Regularization")
plt.pause(1)
plt.savefig(f"L1 sparsity.png")